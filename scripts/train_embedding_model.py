...
- Supports in-batch negatives + optional explicit negatives.
- Uses Sentence-Transformers MultipleNegativesRankingLoss for contrastive fine-tune.
- Optional PEFT LoRA adaptation to reduce VRAM.
...
Example:
  python dev-indexer_1/scripts/train_embedding_model.py \
...
After training you can quantize (optional) via separate script.
"""
        ...
        with open(path,'r') as f:
            for line in f:
                if not line.strip():
                    continue
                obj = json.loads(line)
                a = obj.get('a') or obj.get('text')
                b = obj.get('b') or obj.get('positive')
                if a and b:
                    rows.append((a,b))
...
def apply_lora_if_requested(model: SentenceTransformer, cfg: TrainConfig):
    ...
    if LoraConfig is None:
        ...
        return model
    # Locate the underlying transformer
    ...
    peft_model = get_peft_model(hf_model, lora_conf)
...
def train(cfg: TrainConfig):
    ...
    if os.path.exists(cfg.output):
        if not cfg.overwrite and os.listdir(cfg.output):
