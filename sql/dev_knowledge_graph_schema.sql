...
-- Captures:
--  1. Mission (why)
--  2. Narrative (how: events, decisions, outcomes)
--  3. Codebase (what: source artifacts + semantic/code analysis)
--
-- Notes:
--  * Namespaced potential collisions: existing global tables (model_registry, query_performance)
--    are already defined in retrieval schema. Here we use dev_model_registry, dev_query_performance
--    to avoid structural conflicts. Adjust if you prefer shared registries.
...
CREATE TABLE IF NOT EXISTS project_epics (
  id          BIGSERIAL PRIMARY KEY,
  mission_id  BIGINT REFERENCES project_missions(id) ON DELETE CASCADE,
  name        TEXT NOT NULL,
  description TEXT,
  created_at  TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS source_documents (
  ...
  provenance       JSONB,  -- {"source":"repo|import","origin_repo":...}
  created_at       TIMESTAMPTZ DEFAULT now(),
...
-- 6. MLOPS & SYSTEM HEALTH (Namespaced) -------------------------
...
-- Vector indexes (ANN) with pgvector IVF. Adjust lists with scale.
...
-- 8. OPTIONAL VIEWS / FUTURE EXTENSIONS -------------------------
-- Example: consolidated search surface (narratives + code) for hybrid query.
-- CREATE VIEW dev_search_union AS
-- SELECT id, 'log' AS kind, title AS label, narrative AS body, narrative_embedding AS embedding FROM development_log
-- UNION ALL
-- SELECT id, 'code' AS kind, file_path || ':' || coalesce(chunk_name,'') AS label, code_content AS body, code_embedding FROM code_chunks;

-- End dev_knowledge_graph_schema.sql
